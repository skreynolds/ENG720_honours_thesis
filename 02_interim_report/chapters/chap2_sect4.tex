...
Appling gradient descent to \ref{eq:2401} creates error values which are backpropagated through the network to adjust network weights, as described in \textsection \ref{sec:networktraining}. The problem with this approach is the correlation between the state-action values $Q(s,a)$ and the target values $r + \gamma \max_{a^\prime} Q(s^\prime,a^\prime)$, which makes learning unstable and divergent, similarly to the temporal correlation problem described above. Mnih \textit{et alias} overcame this issue by specifying a second set of network parameters, $\theta^-$, referred to as the target network. The target network parameters, $\theta^-$, are just a copy of the original network parameters $\theta$, that are updated periodically.
