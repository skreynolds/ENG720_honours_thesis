...
Policy search methods are another class of RL algorithms that do not use value functions to determine states to action policy mappings. Figure \ref{fig:2xx_families_of_RL_algorithms} provides a high level overview of policy search methods compared to dynamic programming and value function based methods.
...
Policy search methods use parameterised policies, $\pi_{\theta}(a | s)$. The parameters are represented as a vector, $\theta$, in some vector space, $\Theta$. Changing the elements of $\mathbf{\theta}$ changes how the policy maps states to actions. The basic idea behind policy search methods is that the agent searches directly for the optimal policy in the parameter space, $\Theta$. Suppose the agent possess some policy, $\pi_{\theta}$, which it can roll-out in the environment and get some return, $G$. It can then slightly modify $\theta$ to get some marginally different policy, say $\pi_{\theta'}$. The agent would then roll-out the new policy in the environment and receive a return $G'$. If $G' > G$, the agent would discard the policy $\pi_{\theta}$ in favour of $\pi_{\theta'}$. If $G' < G$, then the agent would retain policy $\pi_{\theta}$. Continued iterations of the above would see the agent's policy converge to an optimal policy, although this is not guaranteed to be globally optimal. 

TALK about the REINFORCE algorithm

%------------------------ SS: Actor Critic Methods
