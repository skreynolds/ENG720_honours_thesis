...
\begin{tikzpicture}	
	...
	% Connect nodes
	\draw [->] (sum4) -- node [at end, label=right:{$U_1(s)$}] {} (c2);
	\draw [->] (r1) -- (sum4);
	\draw [->] (b1) -- (sum6);
	\draw [->] (sum6) -- (int1);
	\draw [->] (int1) -- node [label=above:{$X_1(s)$}] {} (sum4);
	\draw [->] (c8) -| node [at start, label=right:{$\Delta F_1(s)$}] {} (r1);
	\draw [->] (c8) -| (b1);
	\draw [->] (c12) -| node [at start, label=right:{$X_5(s)$}] {} (sum4);
	\draw [->] (c12) -| (sum6);
