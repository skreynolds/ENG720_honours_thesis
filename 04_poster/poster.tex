...
\begin{variableblock}{The Environment in the Frequency \& Temporal Domains}{fg=black,bg=dgreen!10}{fg=white,bg=green!20!gray}
	\begin{figure}
	\resizebox{50cm}{!}{\input{./figures/04_two_area_ode_derivation/two_area_ode_derivation}}
	\end{figure}
...
\begin{variableblock}{Classical PI Controller}{fg=black,bg=dblue!10}{fg=white,bg=blue!20!gray}
	\begin{figure}
		\resizebox{!}{21cm}{\input{./figures/03_classical_pi_controller/classical_pi_controller}}
	\end{figure}
	\begin{center}
		\textbf{ODE System}\\
		\vspace{0.75cm}
...
\begin{block}{Classical PI Controller Results}
content...
\end{block}
...
\begin{variableblock}{DDPG Controller}{fg=black,bg=dblue!10}{fg=white,bg=blue!20!gray}
	...
	\hspace{0.5cm}\begin{algorithm}[H]\DontPrintSemicolon
	Rand. init. critic $Q(s,a|\theta^Q)$ and actor $\mu(s|\theta^\mu)$ with $\theta^Q$ and $\theta^\mu$\;
	
	Init. target networks $Q^\prime$ and $\mu^\prime$ with $\theta^{Q^\prime} \leftarrow \theta^Q$, $\theta^{\mu^\prime} \leftarrow \theta^\mu$\;
	
	Initialise replay buffer $R$\;
	
	\For{$episode \gets 1$ \textbf{to} $M$}{
		
		Initialise a random process $\mathcal{N}$ for action exploration\;
		
		Receive initial observation state $s_1$\;
		
		\For{$t \gets 0$ \textbf{to} $T$}{
			Select action $a_t = \mu(s_t|\theta^\mu) + \mathcal{N}_t$ with noise exploration noise\;
		
			Execute $a_t$ and observe reward $r_t$ and new state $s_{t+1}$\;
		
			Store transition ($s_t$,$a_t$,$r_t$,$s_{t+1}$) in $R$\;
			
			Sample random $N$ transitions ($s_i$,$a_i$,$r_i$,$s_{i+1}$) from $R$\;
			
			Set $y_i = r_i + \gamma Q'(s_{i+1}, \mu'(s_{i+1}|\theta^{Q'}))$\;
			
			Update critic by min. loss: $L = \frac{1}{N} \sum_{i}(y_i - Q(s_i, a_i|\theta^Q))^2$\;
...
\begin{block}{DDPG Controller Results}
