...
In the event that data cannot be acquired from a utility, a synthetic data set may need to be derived. This would be achieved 

\subsection{Theoretical and methodological approach}

Research scope is limited to the following items:
\begin{itemize}
	\item securing load demand profile dataset from major utility, or developing synthetic load profile dataset based on local load profile characteristics;
	\item 
	\item investigating suitable software packages (open source or commercial) to develop simulation;
	\item investigate  
	\item creating a simulated model for the two area power system;
	\item 
	\item development, training and application of the DRL agent to perform the automatic generation control function for a simple two control area power system
\end{itemize}
, either from scratch or using an existing software package. Additionally, different DRL architectures will be explored in order to determine the most appropriate architecture to address the control task. The DRL agent will be tasked with basic load following, and time permitting, restoring the system after a disturbance. Trained DRL controller performance will be measured against a classical linear feedback controller. The key performance aspects that will be compared are the ability to:
\begin{itemize}
