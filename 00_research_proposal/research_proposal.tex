...
At the heart of RL is the functional mapping which converges on the best action to take for a given state, that will ultimately end in the largest cumulative reward for a given episode.
...
A classical control model will need to be developed, using existing control solutions (REFERENCE).

It is anticipated that there may be a number of issues that (face in carrying out your research design. No method is perfect, so you need to describe what the shortcomings will be and explain how you will address them.)
